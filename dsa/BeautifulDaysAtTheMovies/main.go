...
func beautifulDays(i int32, j int32, k int32) (bDays int32) {
	for ; i <= j; i++ {
		abs := int32(math.Abs(float64(i - getReversedNumber(int(i)))))
		if abs%k == 0 {
			bDays++
		}
	}
...
func getReversedNumber(n int) int32 {
	sb := []byte(strconv.Itoa(n))
	for i := 0; i < len(sb)/2; i++ {
		sb[i], sb[len(sb)-i-1] = sb[len(sb)-i-1], sb[i]
	}
	reversed, err := strconv.Atoi(string(sb))
	if err != nil {
		panic(err)
	}
